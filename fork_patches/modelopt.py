...
# --- skinny MoE (SM70 QPN2 routed experts), guarded by VLLM_SKINNY_MOE=1 ---
from vllm.model_executor.layers.quantization import skinny_moe as _skinny_moe
...
class ModelOptNvFp4FusedMoE(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        """
        if _skinny_moe.SKINNY_MOE_ENABLED:
            _skinny_moe.prepack_experts(layer)
            dev = layer.w13_weight.device
            for pn in ('w13_weight', 'w13_weight_scale', 'w2_weight',
                       'w2_weight_scale'):
                replace_parameter(
                    layer, pn,
                    torch.empty(0, device=dev, dtype=torch.uint8))
